...
public class BireyselMusteriTemsilcisiController implements Initializable {
    @FXML
    private Label adSoyadLabel;
    @FXML
    private TextField musteriSayisi;
    @FXML
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        this.bilgileriOku();
        this.guncelle.setVisible(Main.getUser().getAuth().equals("Mudur"));
    }

    public void bilgileriYaz(){
        String yazi = "!" + Main.getSelectedPersonel().getTcKimlikNo() + "@" + this.getAylikSatisHedefi().getText() +
                "%" + this.getToplamSatis().getText() + "&" + this.getMusteriSayisi().getText() + "=" ;

        try {
            if(this.getDy().dosyaVarmi("bireyselMusteriTemsilcisi")) {
                boolean kontrol = false;
                List<String> lines = this.getDy().dosyadanOku("bireyselMusteriTemsilcisi");
                int size = lines.size();
                for (int i = 0; i < size; i++) {
                    ...
                    for (int j = 0; j < strsize; j++) {
                        ...
                        if (tckimlik.toString().equals(String.valueOf(Main.getSelectedPersonel().getTcKimlikNo()))) {
                            ...
                            this.getDy().dosyaGuncelle("bireyselMusteriTemsilcisi", lines);
                ...
                if(!kontrol){
                    ...
                    this.getDy().dosyayaYaz("bireyselMusteriTemsilcisi");
                }
            }
            ...
            for (int i = 0; i < size; i++) {
                ...
                for (int j = 0; j < strsize; j++) {
                    ...
                    if(String.valueOf(Main.getSelectedPersonel().getTcKimlikNo()).equals(tckimlik)){
                        ...
                        this.getMusteriSayisi().setText(String.valueOf(bmt.getToplamMusteriSayisi()));
    ...
    public TextField getMusteriSayisi() {
